...
% erro_quad = (y_ref(round(2*end/10:end)) - y_sys(round(2*end/10:end)))'*(y_ref(round(2*end/10:end)) - y_sys(round(2*end/10:end)));
% erro_quad = (y_ref(round(1:4*end/10)) - y_sys(round(1:4*end/10)))'*(y_ref(round(1:4*end/10)) - y_sys(round(1:4*end/10)));
% erro_quad = (y_ref(round(3*end/4:end)) - y_sys(round(3*end/4:end)))'*(y_ref(round(3*end/4:end)) - y_sys(round(3*end/4:end)));
% erro_quad = (y_ref(round(1*end/2:end)) - y_sys(round(1*end/2:end)))'*(y_ref(round(1*end/2:end)) - y_sys(round(1*end/2:end)));
    %estabiliza em 0.541s

% erro_quad = (y_ref(round(1*end/3:end)) - y_sys(round(1*end/3:end)))'*(y_ref(round(1*end/3:end)) - y_sys(round(1*end/3:end)));
    %estabiliza em 0.347s
    
 %erro_quad = (y_ref(round(2*end/7:end)) - y_sys(round(2*end/7:end)))'*(y_ref(round(2*end/7:end)) - y_sys(round(2*end/7:end)));
    %Estabiliza em 0.303s
